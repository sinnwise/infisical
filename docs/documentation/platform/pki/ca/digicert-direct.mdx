...
- **X9 PKI for TLS** — issues DigiCert X9 PKI for TLS certificates under the `x9_pki` product entitlement. These certificates support public FQDN and IP address configurations, but not wildcard domains.
...
- Entitlement to the product you intend to use (`ssl_plus`, `x9_pki`, `code_signing`, `code_signing_ev`, ...) on your CertCentral account. Contact DigiCert to enable X9 PKI for TLS if it is not listed in your products.
...
<Tabs>
  <Tab title="Infisical UI">
    <Steps>
      ...
      <Step title="Create the External CA">
        ...
        - **Purpose** — `SSL / TLS`, `X9 PKI for TLS`, or `Code Signing`. The product list filters accordingly.
        - **Product** — the CertCentral entitlement this CA will issue under
        - **Domain Validation Scope** (X9 PKI for TLS only) — choose whether DigiCert validates at the base-domain level, the exact FQDN level, or uses your CertCentral account default.
  ...
  <Tab title="API">
    ...
    ```bash Create a DigiCert CA (X9 PKI for TLS)
    curl --request POST \
      --url https://app.infisical.com/api/v1/pki/ca/digicert \
      --header 'Content-Type: application/json' \
      --data '{
...
## X9 PKI for TLS requirements

X9 PKI for TLS certificates support only fully qualified domain names and public IP addresses. They do not support wildcard domains such as `*.example.com`.

For RSA certificates, choose SHA-256, SHA-384, or SHA-512. DigiCert selects SHA-256 for P-256 ECDSA certificates and SHA-384 for P-384 ECDSA certificates.

For X9 PKI for TLS requests, Infisical accepts the following key usages and extended key usages:

- Key usage: Digital Signature and Key Agreement / Encipherment
- Extended key usage: Server Authentication and Client Authentication

For an order that includes an IP address, Infisical selects DigiCert's HTTP Practical Demonstration validation method. DigiCert requires this method for IP address validation. Complete the validation steps in CertCentral before DigiCert issues the certificate.
...
| **Failed** | If DigiCert does not issue within 24 hours. X9 PKI for TLS requests remain pending for up to 30 days to allow you to complete domain control validation. |
